...
- **着手時の記録（2026-08-12）**：成果物 `nodered-plc-register-guide.html`（13 節）。既存 2 ガイドを
  実測して重複範囲を確定した（`modbus` はデータモデルとアドレス体系、`buffer-parser` の 3 章は
  エンディアンとスワップの仕組み）。そのため C は**症状から解釈のずれを特定する**側に寄せ、
  「症状から原因を引く」早見表を軸に、符号・ワード順（ABCD / CDAB / BADC / DCBA の業界表記）・
  スケーリング・ビットと BCD の 4 観点で構成した。仕組みの説明は buffer-parser 3 章へ、
  アドレス体系は modbus へリンクで送っている。
  - **環境バッジは ☁️**：サンプルフローを標準ノード（inject / function / debug / comment）だけで
    組み、レジスタ値は inject で模擬する。実機も追加パッケージも要らない。掲載した数値と
    Function の出力はすべて Node.js で実測して突き合わせた。
  - **相互リンク**：`modbus`（用語集エンディアン）、`buffer-parser`（次に読むとよいガイド）、
    `variable-types` Part 1（Buffer 節）の 3 箇所から参照を張った。
